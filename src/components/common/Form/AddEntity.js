...
const AddEntity = () => {
  ...
  const dependency = useSelector(
    (state) => state?.dependency?.dependencies?.[type]
  );

  const saveForm = (formState) => {
    let query = {};
    let queryFields = entityModel[type].fields.filter(
      (field) => field.dependencyField
    );
